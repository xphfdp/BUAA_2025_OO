...
public class NetworkTest {
    ...
    public void testQueryCoupleSum() {
        PersonInterface[] oldPersons = ((Network) oldNetwork).getPersons();
        int stdAnswer = calculateCoupleSum(oldPersons);
        int testAnswer = newNetwork.queryCoupleSum();
        assertEquals("Couple sum mismatch", stdAnswer, testAnswer);
    ...
    @Test
    public void testCoupleNetwork() {
        NetworkInterface network = new Network();
        try {
            network.addPerson(new Person(0, "A", 20));
            network.addPerson(new Person(1, "B", 21));
            network.addRelation(0, 1, 100);
            assertEquals("Single couple network should have 1 couple", 1, network.queryCoupleSum());
        } catch (Exception e) {
            fail("Setup failed: " + e.getMessage());
        }
    }
    ...
    @Test
    public void testTripleRelationModification() {
        NetworkInterface network = new Network();
        try {
            network.addPerson(new Person(0, "A", 20));
            network.addPerson(new Person(1, "B", 21));
    ...
    @Test
    public void testCoupleRelationModification() {
        NetworkInterface network = new Network();
        try {
            network.addPerson(new Person(0, "A", 20));
            network.addPerson(new Person(1, "B", 21));
            network.addPerson(new Person(2, "C", 22));
            network.addRelation(0, 1, 100); // A-B 形成情侣
            assertEquals("Initial network should have 1 couple", 1, network.queryCoupleSum());
            network.addRelation(0, 2, 200); // A-C 价值更高，破坏A-B情侣
            assertEquals("After adding stronger relation, network should have 1 couples", 1, network.queryCoupleSum());
            network.modifyRelation(0, 2, -200); // 移除A-C关系，恢复A-B情侣
            assertEquals("After removing stronger relation, network should have 1 couple", 1, network.queryCoupleSum());
        } catch (Exception e) {
            fail("Setup failed: " + e.getMessage());
        }
    }
     ...
     * 计算社交网络中情侣关系网的数量
     */
    private int calculateCoupleSum(PersonInterface[] persons) {
        int count = 0;
        for (int i = 0;i < persons.length;i++) {
            for (int j = i +1;j < persons.length;j++) {
                try {
                    int bestId1 = oldNetwork.queryBestAcquaintance(persons[i].getId());
                    int bestId2 = oldNetwork.queryBestAcquaintance(persons[j].getId());
                    if (bestId1 == persons[j].getId() && bestId2 == persons[i].getId()) {
                        count++;
                    }
    ...
    private static void generateTriangleNetwork(NetworkInterface oldNetwork,
        ...
        try {
            ...
            // 添加情侣关系
            oldNetwork.addRelation(3, 4, 100); // 高价值，确保3-4互为最佳熟人
            newNetwork.addRelation(3, 4, 100);
            // 添加随机关系
            for (int i = 0; i < 50; i++) {
    ...
    private static void generateRandomNetwork(NetworkInterface oldNetwork,
        ...
        try {
            ...
            int id2;
            do {
                id2 = random.nextInt(n);
            } while (id2 == id1);
            oldNetwork.addRelation(id1, id2, 200); // 高价值确保情侣
            newNetwork.addRelation(id1, id2, 200);
        } catch (EqualRelationException | PersonIdNotFoundException e) {
            // 忽略异常
        }
    }
