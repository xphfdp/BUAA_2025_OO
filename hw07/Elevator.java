...
public class Elevator extends Thread {
    ...
    private boolean isDoubleMode = false; // 是否处于双轿厢模式
    private int minFloor = Constants.MIN_FLOOR_NUM;      // 运行范围下限
    private int maxFloor = Constants.MAX_FLOOR_NUM;       // 运行范围上限
    private boolean nowInUpdate;
    private int sharedFloor;
    ...
    public void update() {
        setNowInUpdate(true);
        Update updateRequest = eleRequest.getUpdateRequest();
        int elevatorAId = updateRequest.getElevatorAId();
        int elevatorBId = updateRequest.getElevatorBId();
        TransferFloor transferFloor = updateRequest.getTransferFloor();
        if (elePersonNum != 0) {
            personTemOutEle();
        }
        ...
        int sharedFloor = transferFloor.getTransferFloor();
        if (id == elevatorAId) {
            this.speed = Constants.NEW_SPEED;
            this.minFloor = sharedFloor;
            this.maxFloor = Constants.MAX_FLOOR_NUM;
            this.eleFloor = sharedFloor + 1;
            this.isDoubleMode = true;
            this.sharedFloor = sharedFloor;
            this.transferFloor = transferFloor;
        } else if (id == elevatorBId) {
            this.speed = Constants.NEW_SPEED;
            this.minFloor = Constants.MIN_FLOOR_NUM;
            this.maxFloor = sharedFloor;
            this.eleFloor = sharedFloor - 1;
            this.isDoubleMode = true;
            this.sharedFloor = sharedFloor;
            this.transferFloor = transferFloor;
        ...
        setNowInUpdate(false);
        eleRequest.setUpdateRequest(null);
    }
    ...
    public void setNowInUpdate(boolean nowInUpdate) {
        this.nowInUpdate = nowInUpdate;
    }

    public boolean isNowInUpdate() {
        return nowInUpdate;
    }
    ...
    public boolean isFree() {
        return !isNowInSche() && !isNowInUpdate();
    }
